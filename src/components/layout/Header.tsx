    ...
    <header className="h-12 bg-white/95 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800 px-4 flex items-center justify-between select-none backdrop-blur-md sticky top-0 z-30 transition-colors duration-200">
      ...
      <div className="flex items-center gap-3">
        <div className="relative group">
          <div className="flex items-center gap-2.5 px-3 py-1 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200/80 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700/80 rounded-lg cursor-pointer transition-all duration-200 shadow-sm">
            ...
            <div>
              <div className="text-xs font-bold text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
                ...
                <ChevronDown className="w-3 h-3 text-slate-500 dark:text-slate-400 group-hover:rotate-180 transition-transform duration-200" />
              </div>
            </div>
          </div>

          {/* Client Selection Dropdown Menu */}
          <div className="absolute top-full left-0 mt-1.5 w-80 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/90 rounded-2xl shadow-2xl z-50 hidden group-hover:block p-2 space-y-1 backdrop-blur-xl">
            <div className="px-3 py-1.5 text-[10px] font-extrabold text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800 mb-1">
              Danh Sách Khách Hàng Kế Toán ({clients.length})
            </div>
            {clients.map((c) => (
              <button
                key={c.id}
