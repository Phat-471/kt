...
export const ContributionFeeTab: React.FC<ContributionFeeTabProps> = ({
  ...
  const [activeYear, setActiveYear] = useState<number>(initialYear || 2026);
  ...
  // Danh sách các năm có sẵn từ dữ liệu periods
  const availableYears = useMemo(() => {
    const yearsSet = new Set<number>([2026, 2025, 2024, 2023, initialYear]);
    periods.forEach(p => {
      if (p.year && p.year >= 2020 && p.year <= 2030) yearsSet.add(p.year);
    });
    return Array.from(yearsSet).sort((a, b) => b - a);
  }, [periods, initialYear]);

  // Key của tháng đang chọn (VD: "062026")
  const currentMonthKey = `${String(selectedMonth).padStart(2, '0')}${activeYear}`;

  // Tìm period của tháng hiện tại
  const currentMonthPeriod = useMemo(() => {
    const found = periods.find(p => p.periodKey === currentMonthKey || (p.month === selectedMonth && p.year === activeYear));
    if (found) return found;
    return null;
  }, [periods, currentMonthKey, selectedMonth, activeYear]);

  // Kỳ Quý được tính tự động từ các tháng
  const quarterlyPeriod = useMemo(() => {
    return generateQuarterlyContributionPeriod(selectedQuarter, activeYear, periods);
  }, [selectedQuarter, activeYear, periods]);

  // Bảng Tổng hợp Cả Năm (Sheet TC)
  const yearSummaryTC = useMemo(() => {
    return generateYearSummaryTC(activeYear, periods, activeClient || undefined);
  }, [activeYear, periods, activeClient]);
  ...
  const handleInitializeFromEmployees = () => {
    ...
    const newPeriod = recalculateContributionPeriod({
      ...
      periodLabel: `Tháng ${String(selectedMonth).padStart(2, '0')}/${activeYear}`,
      periodType: 'MONTH',
      year: activeYear,
      month: selectedMonth,
      reportDate: `Ngày 01 tháng ${String(selectedMonth).padStart(2, '0')} năm ${activeYear}`,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        ...
        {/* 3 Nút Chọn Chế Độ Xem & Bộ Lọc Năm */}
        <div className="flex items-center gap-2 flex-wrap w-full lg:w-auto">
          ...
          <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 flex-1 lg:flex-none">
            <button
          ...
          <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1 overflow-x-auto w-full md:w-auto pb-1 md:pb-0">
              {Array.from({ length: 12 }, (_, i) => i + 1).map(m => {
                ...
                const monthKey = `${String(m).padStart(2, '0')}${activeYear}`;
                const hasData = periods.some(p => p.periodKey === monthKey || (p.month === m && p.year === activeYear));
          ...
          {!currentMonthPeriod ? (
            <div className="bg-white p-8 rounded-xl border border-slate-200 text-center space-y-3 shadow-sm">
              ...
              <div className="font-bold text-slate-800 text-sm">
                Chưa có dữ liệu Bảng Trích Nộp Tháng {selectedMonth}/{activeYear}
              </div>
            ...
            <div className="space-y-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
              ...
              <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-2">
                ...
                <div className="text-right font-semibold text-slate-700">
                  <div>{currentMonthPeriod.reportDate || `Ngày 01 tháng ${String(selectedMonth).padStart(2, '0')} năm ${activeYear}`}</div>
      ...
      {viewMode === 'YEAR_TC' && (
        <div className="space-y-4">
          <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              <div className="font-bold text-slate-800 text-sm">Bảng Tính Kinh Phí, Đoàn Phí Công Đoàn Năm {activeYear} (Mẫu TC)</div>
      ...
      {isAddMemberModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-5 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            <h3 className="font-bold text-slate-900 text-base mb-1">Thêm Nhân Viên Vào Bảng Tháng {selectedMonth}/{activeYear}</h3>
