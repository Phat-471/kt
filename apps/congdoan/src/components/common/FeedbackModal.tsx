...
export const FeedbackModal: React.FC<FeedbackModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleExportDiagnostics = async () => {
    try {
      const txCount = await db.unionTransactions.count();
      const periodCount = await db.unionContributionPeriods.count();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Lưu vào Local Storage & Gửi báo cáo
    try {
      ...
      {isSuccess ? (
        <div className="text-center py-6 space-y-4">
          ...
          <div className="p-3.5 bg-blue-50 rounded-xl border border-blue-200 text-xs text-left text-slate-700 space-y-1.5">
            ...
            <div>• <strong>Hotline / Zalo:</strong>0974.194.305</div>
